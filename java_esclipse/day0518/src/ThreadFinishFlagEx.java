...
class RandomThread extends Thread {
	...
	public void run() {
		while(true) { 
			...
			label.setSize(80, 30); //레이블 크기 80, 30
		...
		//라벨을 붙이고 뿌리고 계속 반복
			try {
				...
				if(flag==true) {
					contentPane.removeAll();//컨텐트에 있는 모든 레이블 제거
					label = new JLabel("finish"); 
					label.setSize(80, 30); //레이블 크기
					label.setLocation(100, 100); //레이블을 컨텐트팬 내의 임의의 위치로 설정
					label.setForeground(Color.RED);//색 변경
					contentPane.add(label); //레이블 추가
					contentPane.repaint(); //화면 클릭했을 때 바뀌는 화면
					return; // 스레드 종료
				}
			}
...
public class ThreadFinishFlagEx extends JFrame {
	...
	public ThreadFinishFlagEx() {
		...
		c.addMouseListener(new MouseAdapter() {//마우스 리스너
			@Override
